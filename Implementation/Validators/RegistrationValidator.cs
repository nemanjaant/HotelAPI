...
namespace Implementation.Validators
{
    public class RegistrationValidator : AbstractValidator<UserDTO>
    {
        ...
        public RegistrationValidator(HotelContext context)
        {
            RuleFor(x => x.Email)
                .Cascade(CascadeMode.Stop)
                .NotEmpty().WithMessage("Email je obavezan podatak.")
                .EmailAddress().WithMessage("Email nije ispravnog formata.")
                .Must(x => !context.Users.Any(u => u.Email == x)).WithMessage("Email adresa {PropertyValue} je već u upotrebi.");
            ...
            RuleFor(x => x.Username)
                ...
                .Must(x => !context.Users.Any(u => u.Username == x)).WithMessage("Korisničko ime {PropertyValue} je već u upotrebi.");
